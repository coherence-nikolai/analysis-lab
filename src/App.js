...
import { Play, Brain, GitBranch, MessageSquare, Microscope, Target, CheckCircle, XCircle, Lightbulb, ArrowRight, Volume2, VolumeX } from ‘lucide-react’;
...
const scenarios = [
  {
    ...
    correctExplanation: "Multiple factors contribute: population growth leads to more cars, while inadequate public transport and road construction worsen the problem."
  },
  {
    ...
    correctExplanation: "The new method works through engagement, clarity, and practice, leading to better understanding."
  }
];
...
const checkAnswer = () => {
  ...
  const isCorrect = correct.length === selectedItems.length && 
                   correct.every(id => selectedItems.includes(id));
...
const cases = [
  {
    ...
    options: [
      { 
        ...
        isCorrect: true,
        explanation: "The argument assumes breakfast causes better scores, but correlation doesn't prove causation. Other factors (family income, sleep, study habits) might explain both breakfast eating and test performance."
      },
      { 
        id: 2, 
        text: "Valid causal inference", 
        isCorrect: false,
        explanation: "Without controlling for other variables, we can't conclude breakfast directly causes better scores."
      },
      { 
        id: 3, 
        text: "Appeals to emotion only", 
        isCorrect: false,
        explanation: "The argument uses evidence (studies), not just emotion."
      }
    ]
  },
  {
    ...
    question: "What fallacy is present here?",
    options: [
      { 
        id: 1, 
        text: "Anecdotal evidence / Small sample", 
        isCorrect: true,
        explanation: "Personal observations aren't representative. Many successful people have different schedules, and correlation doesn't prove waking up early causes success."
      },
      { 
        id: 2, 
        text: "Sound logical reasoning", 
        isCorrect: false,
        explanation: "This is flawed reasoning based on limited personal experience."
      },
      { 
        id: 3, 
        text: "Circular reasoning", 
        isCorrect: false,
        explanation: "The conclusion doesn't restate the premise - it's different (though flawed) reasoning."
      }
    ]
  }
];
...
return (
  <div className="space-y-6 animate-fade-in">
    <div className="bg-gradient-to-r from-purple-500/10 to-pink-600/10 p-6 rounded-2xl border border-purple-500/30">
      <h3 className="text-2xl font-bold text-purple-400 mb-4">Case {currentCase + 1}: Analyze the Argument</h3>
...
const systems = [
  {
    ...
    explanation: "In a forest ecosystem, everything is interconnected. Trees need water, sunlight, and soil nutrients. Animals depend on trees and water. Decomposers break down organic matter to enrich soil, which feeds trees. It's a cycle!"
  }
];
...
const toggleConnection = (from, to) => {
  ...
  setConnections(prev => {
    ...
    if (newConnections[key] || newConnections[reverseKey]) {
      delete newConnections[key];
      delete newConnections[reverseKey];
    } else {
      newConnections[key] = true;
    }
...
return (
  <div className="space-y-6 animate-fade-in">
    <div className="bg-gradient-to-r from-orange-500/10 to-red-600/10 p-6 rounded-2xl border border-orange-500/30">
      ...
      <p className="text-gray-200">Click on two components to show how they interact:</p>
    </div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
      {system.components.map((component, idx) => (
        ...
        >
          <button
            className="w-full p-6 bg-gradient-to-br from-orange-500/20 to-red-600/20 rounded-2xl border-2 border-orange-500/50 hover:scale-105 transition-all duration-300 font-bold text-white text-lg shadow-lg hover:shadow-orange-500/30"
            style={{
    ...
    <div className="bg-gray-800/50 p-5 rounded-xl border border-gray-700">
      <h4 className="text-orange-300 font-semibold mb-3">Instructions:</h4>
      <p className="text-gray-300 text-sm">In a real system, these components would have many connections. For this exercise, think about how each part affects others. When ready, click "Show System Map" to see the interconnections explained.</p>
    </div>
...
return (
  <div className="space-y-6 animate-fade-in">
    ...
    <div className="bg-gradient-to-br from-emerald-500/20 to-green-600/20 p-6 rounded-2xl border-2 border-emerald-500/50">
      ...
      <div className="space-y-8 animate-fade-in">
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <Icon className={`bg-gradient-to-r ${module.color} bg-clip-text text-transparent group-hover:scale-110 transition-transform`} size={48} />
        ...
        <div className="bg-gradient-to-r from-gray-800/50 to-gray-700/50 p-8 rounded-3xl border border-gray-700 text-center">
          ...
          <p className="text-gray-300 text-lg font-light max-w-3xl mx-auto">
            ...
            recognize patterns in complex systems, and test ideas with evidence. Master them, and you'll make better decisions in every area of life.
          </p>
        </div>
      </div>
    )}
