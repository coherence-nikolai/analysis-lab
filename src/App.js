...
const AnalysisLab = () => {
  ...
  const styles = {
    app: {
      ...
      // ✅ FIX: allow scrolling (your old code used overflow: 'hidden')
      overflowY: "auto",
          ...
          {scenarios.map((_, i) => (
            <div
              ...
              style={{
                ...
                background:
                  i < current ? "#22c55e" : i === current ? "#06b6d4" : "rgba(255,255,255,0.2)",
                transition: "all 0.3s"
              }}
            />
          ))}
          ...
          {cases.map((_, i) => (
            <div
              ...
              style={{
                ...
                width: i === current ? "40px" : "25px",
                background:
                  i < current ? "#22c55e" : i === current ? "#a855f7" : "rgba(255,255,255,0.2)",
                transition: "all 0.3s"
              }}
            />
          ))}
  ...
  const SystemsGame = () => {
    ...
    const systems = [
      {
        ...
        keyInsight:
          "Everything is interconnected! Trees take nutrients from soil and provide food for animals. Animals spread seeds and create waste that decomposers break down into nutrients. Sunlight powers the whole system through photosynthesis. Disrupting ANY part affects EVERYTHING else!"
      },
      {
        ...
        keyInsight:
          "Schools are complex systems! Funding affects resources which impacts teaching quality. Engaged students motivate teachers and reassure parents, who advocate for better funding. When one part struggles, it ripples through everything else."
      }
    ];
    ...
    return (
      <div>
          ...
          {systems.map((_, i) => (
            <div
              ...
              style={{
                ...
                width: i === current ? "40px" : "25px",
                background:
                  i < current ? "#22c55e" : i === current ? "#f97316" : "rgba(255,255,255,0.2)",
                transition: "all 0.3s"
              }}
            />
          ))}
        ...
        <div style={styles.card}>
          <div style={{ color: "#f97316", fontWeight: "bold", marginBottom: "10px" }}>
            System {current + 1}: {system.name}
          </div>
          <div style={{ color: "#94a3b8" }}>{system.description}</div>
          {firstSelected && (
            <div style={{ color: "#f97316", fontWeight: "bold", marginTop: "10px" }}>
              Selected: {system.components.find((c) => c.id === firstSelected).label} - now tap another
            </div>
          )}
        </div>
        ...
        {selectedPairs.length > 0 && !revealed && (
          <div style={{ ...styles.card, marginBottom: "15px" }}>
            <div style={{ color: "#f97316", fontWeight: "bold", marginBottom: "8px" }}>
              Your connections ({selectedPairs.length}):
            </div>
            {selectedPairs.map((pair) => {
              ...
              return (
                <div key={pair} style={{ color: "#cbd5e1", fontSize: "0.9rem", marginBottom: "4px" }}>
                  {compA.label} ↔ {compB.label}
                </div>
              );
            })}
          </div>
            ...
            <div style={{ marginBottom: "10px" }}>
              {system.components.map((comp) => (
                <div key={comp.id} style={{ color: "#cbd5e1", fontSize: "0.9rem", marginBottom: "4px" }}>
                  {comp.label} affects:{" "}
                  {comp.affects.map((id) => system.components.find((c) => c.id === id).label).join(", ")}
                </div>
              ))}
            </div>

            <button onClick={next} style={styles.nextBtn}>
              {current < systems.length - 1 ? "Next System →" : "Complete Module ✓"}
            </button>
          </div>
        )}
      </div>
  ...
  const ScientificGame = () => {
    ...
    const nextStep = () => {
      if (step < experiment.steps.length - 1) {
        setStep((s) => s + 1);
        setFeedback(null);
      } else if (current < experiments.length - 1) {
        setCurrent((c) => c + 1);
        setStep(0);
        setAnswers({});
        setFeedback(null);
    ...
    return (
      <div>
        ...
        {currentStep.options.map((option, idx) => {
          let bg = "rgba(255,255,255,0.05)";
          let border = "2px solid rgba(255,255,255,0.15)";
          if (selectedAnswer === option.text && !feedback) {
            bg = "rgba(34,197,94,0.2)";
            border = "2px solid #22c55e";
          }
          if (feedback && option.correct) {
          ...
          if (feedback && !option.correct && selectedAnswer === option.text) {
            bg = "rgba(239,68,68,0.2)";
            border = "2px solid #ef4444";
          }
          return (
            <button key={idx} onClick={() => select(option.text)} style={{ ...styles.optionBtn, background: bg, border }}>
              {option.text}
            </button>
          );
        })}
        ...
        {!feedback ? (
          <button
            ...
            style={{ ...styles.submitBtn, opacity: !selectedAnswer ? 0.5 : 1, background: "linear-gradient(90deg, #22c55e, #06b6d4)" }}
          >
            Submit Answer
          </button>
        ) : (
          <div style={{ ...styles.feedback, background: feedback.correct ? "rgba(34,197,94,0.15)" : "rgba(249,115,22,0.15)", border: "1px solid " + (feedback.correct ? "rgba(34,197,94,0.4)" : "rgba(249,115,22,0.4)") }}>
            <div style={{ fontSize: "1.2rem", fontWeight: "bold", marginBottom: "10px" }}>
              {feedback.correct ? "🎉 Correct! +10 points" : "💡 Here is the scientific reasoning:"}
            </div>
  ...
  const renderGame = () => {
    switch (selectedModule) {
      ...
      case "systems":
        return <SystemsGame />;
      case "scientific":
        return <ScientificGame />;
      default:
        return null;
    }
  };

  // ✅ Sound toggle that unlocks audio on iOS from a user gesture
  ...
  // Optional: prevent memory leaks on unmount
  useEffect(() => {
    return () => {
      try {
        ...
        {currentScreen === "home" && (
          <div>
            ...
            <div style={{ ...styles.card, textAlign: "center" }}>
              <h3 style={{ fontSize: "1.3rem", marginBottom: "10px", color: "#06b6d4" }}>
                Why Critical Thinking Matters
              </h3>
              <p style={{ color: "#94a3b8", lineHeight: 1.7 }}>
                These four skills help you understand the world deeply: trace how events connect,
                evaluate arguments carefully, recognise patterns in complex systems, and test ideas
                with evidence.
              </p>
            </div>
          </div>
