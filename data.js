...
var TRANSLATIONS = {
  en: {
    ...
    obsCount: function(n) { return n > 0 ? "You have consciously observed " + (n) + " time" + (n!==1?'s':'') : ''; },
    obsFirst: function(s) { return "You have observed <strong>" + (s) + "</strong> for the first time.\\nThe wave function just found a new direction."; },
    obsMany: function(s,n) { return "You have observed <strong>" + (s) + "</strong> " + (n) + " times.\\nThis collapse is becoming familiar.\\nAt some point — it will simply be you."; },
    closings: [
    ...
    breathExhale: function(s) { return "Exhale into " + (s); },
    breathHold: 'Hold...',
    breathCycles: function(n,t) { return (n) + " of " + (t); },
    breathEnd: function(s) { return "Your nervous system now knows the direction.\\n<strong>" + (s) + "</strong> is encoded in you.\\nKeep observing."; },
  },
  es: {
    ...
    obsCount: function(n) { return n > 0 ? "Has observado conscientemente " + (n) + " vez" + (n!==1?'es':'') : ''; },
    obsFirst: function(s) { return "Has observado <strong>" + (s) + "</strong> por primera vez.\\nLa función de onda encontró una nueva dirección."; },
    obsMany: function(s,n) { return "Has observado <strong>" + (s) + "</strong> " + (n) + " veces.\\nEste colapso se está volviendo familiar.\\nEn algún momento — simplemente serás tú."; },
    closings: [
    ...
    breathExhale: function(s) { return "Exhala hacia " + (s); },
    breathHold: 'Sostén...',
    breathCycles: function(n,t) { return (n) + " de " + (t); },
    breathEnd: function(s) { return "Tu sistema nervioso ahora conoce la dirección.\\n<strong>" + (s) + "</strong> está codificado en ti.\\nSigue observando."; },
  }
};
...
function getImagination(lang, stateName) {
  var pool = IMAGINATION[lang][stateName];
  if (!pool) return '';
